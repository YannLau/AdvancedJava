...
在 Java 中数据类型分为基本数据类型和引用数据类型。<mark>基本数据类型由虚拟机预先定义，引用数据类型则需要进行类(包括接口、注解、枚举等)的加载。</mark>
...
$\color{red}{所谓加载，简而言之就是将Java类的字节码文件加载到机器内存中，并在内存中构建出Java类的原型——类模板对象。}$所谓类模板对象，其实就是 Java 类在 JVM 内存中的一个快照，JVM 将从字节码文件中解析出的常量池、类字段、类方法等信息存储到类模板中，这样 JVM 在运行期便能通过类模板而获取 Java 类中的任意信息，能够对 Java 类的成员变量进行遍历，也能进行 Java 方法的调用。
...
- 解析类的二进制数据流为方法区(元空间)内的数据结构（Java 类模型）

- （在堆区）创建 java.lang.Class 类的实例，表示该类型。作为方法区这个类的各种数据的访问入口
...
类将.class 文件加载至元空间后，会在堆中创建一个 Java.lang.Class 对象，用来封装类位于方法区内的数据结构，该 Class 对象是在加载类的过程中创建的，每个类都对应有一个 Class 类型的对象。(instanceKlass --›mirror : Class的实例)

> Java中的instanceKlass是什么，在JVM内存中的那个位置存储？
>
> ###### ChatGPT 说：
>
> `instanceKlass` 是 JVM 中用于表示 Java 类的一个内部数据结构，主要在 HotSpot JVM 中使用。它是 JVM 类加载器在内存中存储 Java 类的核心部分之一，主要包含类的元数据（metadata），如类的常量池、字段、方法、接口信息等。
>
> ### `instanceKlass` 的作用：
>
> `instanceKlass` 结构体包含了与 Java 类相关的各种信息，具体包括：
>
> - 类名
> - 父类和接口的信息
> - 常量池（Constant Pool）
> - 类的字段和方法的信息
> - 类的访问标志（如 `public`, `abstract`, `final` 等）
> - 静态变量
>
> ### `instanceKlass` 在 JVM 内存中的位置
>
> `instanceKlass` 存储在 JVM 的 **方法区（Method Area）** 中。在 JDK 8 及之前的版本中，方法区是 **永久代（Permanent Generation, PermGen）** 的一部分，专门用来存放类的元数据。而在 JDK 8 之后，永久代被移除，方法区的实现被称为 **元空间（Metaspace）**。元空间是用本地内存（Native Memory）来存储类的元数据。
>
> 因此，`instanceKlass` 所有的类元数据都存储在元空间中。
>
> ### JVM 内存模型与 `instanceKlass` 的位置
>
> 在 JVM 的内存模型中，`instanceKlass` 的存储位置如下：
>
> 1. **方法区（Method Area）**：用于存储类的元数据，比如类的定义、方法、字段等信息。而 `instanceKlass` 就是存储在方法区中的类元数据结构之一。
> 2. **元空间（Metaspace）**：从 JDK 8 开始，元空间替代了永久代存储类的元数据，而 `instanceKlass` 就是存放在元空间中的。
>
> 总结来说，`instanceKlass` 是 JVM 中的类元数据结构，存储在方法区（JDK 8 以后具体是在元空间）。
...
- $\color{red}{这里不包含基本数据类型的字段用static\quad final修饰的情况，因为final在编译的时候就会分配了，准备阶段会显式赋值。}$

  ```java
  // 一般情况：static final修饰的基本数据类型、字符串类型字面量会在准备阶段赋值
  private static final String str = "Hello world";
  // 特殊情况：static final修饰的引用类型不会在准备阶段赋值，而是在初始化阶段(clinit)赋值
...
• 一个类中包含static fina1修饰的基本数据类型的字段，这些类字段初始化语句采用编译时常量表达式
...
3. <mark>静态字段</mark>：当使用类、接口的静态字段时（final 修饰特殊考虑），比如，使用 getstatic 或者 putstatic 指令。（对应访问变量、赋值变量操作）    `这里的特殊考虑和前面的static final 修饰小节有关系。`如果值在没有执行clinit方法就能拿到，那么就不会进入初始化阶段
   ...
   interface CompareA{
       ...
       {
       	system.out.printIn（"CompareA的初始化"）；
       }
     }
   public static final int NUM1 = 1;    //访问这个不会打印  CompareA的初始化
   public static final int NUM2 = new Random().nextInt( bound: 10); //访问这个会打印  CompareA的初始化
   ```
...
<p style="color:red">常量池中废弃的常量 和 不再使用的类型</p>

HotSpot 虚拟机对常量池的回收策略是很明确的，只要常量池中的常量没有被任何地方引用，就可以被回收。

判定一个常量是否“废弃”还是相对简单，而要判定一个类型是否属于“不再使用的类”的条件就比较苛刻了。需要同时满足下面三个条件：

- $\color{yellow}{该类所有的实例都已经被回收。也就是Java堆中不存在该类及其任何派生子类的实例。}$
- $\color{yellow}{加载该类的类加载器已经被回收。这个条件除非是经过精心设计的可替换类加载器的场景，如OSGi、JSP的重加载等，否则通常是很难达成的。}$
- $\color{yellow}{该类对应的java.lang.Class对象没有在任何地方被引用，无法在任何地方通过反射访问该类的方法。}$
...
> public class final1 {
>
> ​	public static final final2 FINAL2 = new final2(); 
>
> } 
>
> public class final2 {    
>
> ​	public static final final1 FINAL1 = new final1(); 
>
> } 
>
> 上面的两个类中的final属性的赋值应该是发生在类的初始化阶段的clinit方法中，但是此时对方都还没有初始化完成，为什么不会报错？
>
> ###### ChatGPT 说：
>
> 在你给出的两个类 `final1` 和 `final2` 中，它们相互依赖地初始化了对方的 `static final` 属性。从表面上看，这似乎会导致初始化时的循环依赖问题，但实际上，在 Java 的类初始化过程中，并不会产生你预期的错误。我们可以深入到类加载和初始化的机制来解释原因。
>
> ### 1. 类初始化的机制
>
> Java 类初始化的逻辑是由 JVM 执行的，其中类的静态变量和静态代码块都在类初始化阶段进行。类初始化的步骤主要如下：
>
> 1. **加载**：将类的 `.class` 文件加载到 JVM 中，生成类的元信息。
>
> 2. 链接
>
>    ：
>
>    - **验证**：确保类文件的字节码格式正确，并符合 JVM 规范。
>    - **准备**：为类的静态变量分配内存，并将它们初始化为默认值（即数字类型为 `0`，引用类型为 `null`）。
>    - **解析**：将常量池中的符号引用解析为直接引用。
>
> 3. **初始化**：执行类的 `<clinit>` 方法，完成静态变量和静态代码块的赋值。
>
> ### 2. 你的类的初始化顺序
>
> - 当 `final1` 被首次访问时，JVM 开始初始化 `final1` 类。
> - 在初始化 `final1` 类的 `FINAL2` 属性时，需要实例化 `final2` 类，因此 `final2` 的初始化开始。
> - 在 `final2` 的初始化过程中，`FINAL1` 属性需要被赋值为一个 `final1` 实例，但此时 JVM 已经标记 `final1` 的类正在被初始化。
> - JVM 会在静态初始化时确保不会重复初始化一个类，因此在 `final2` 初始化时，它会得到已经正在初始化的 `final1` 类的一个未完全初始化的实例引用。这是一种安全的引用，并且不会触发错误。
>
> ### 3. 为什么不会报错
>
> 这是因为 JVM 会为类的静态变量先分配内存空间并初始化为默认值。在你的场景中，类 `final1` 和 `final2` 中的 `FINAL1` 和 `FINAL2` 静态变量，在初始化之前都会先被设置为 `null`。一旦 JVM 发现一个类正在初始化，它允许继续进行初始化操作，而不会引发死循环或空指针异常。
>
> - 当 JVM 在初始化 `final1` 时，它发现 `final2` 还没有初始化，于是开始初始化 `final2`。
> - 在 `final2` 的初始化过程中，它会去引用 `final1` 的 `FINAL1`，但是 JVM 发现 `final1` 已经在初始化中，因此会直接返回一个安全的（虽然未完全初始化的）引用。
> - 这个过程中，类加载器的设计保证了不会出现由于依赖导致的死锁或递归初始化错误。
>
> ### 总结
>
> - JVM 在初始化类时，会先为类的静态变量分配内存并设置为默认值，因此可以在类的初始化过程中安全地引用正在初始化的对象。
> - 类的初始化过程是有序的，即便存在相互依赖的静态变量，也不会导致错误或循环依赖。
> - 由于 `final1` 和 `final2` 的初始化涉及相互依赖，但 JVM 已经处理了这种情况，因此不会报错。
