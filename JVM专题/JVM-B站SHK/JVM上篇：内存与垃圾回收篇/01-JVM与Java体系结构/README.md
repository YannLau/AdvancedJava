...
<img src="https://img-blog.csdnimg.cn/img_convert/555790a8a0b57075bdc8b6116545e461.png" alt="image-20210507095948516" style="zoom:50%;" />

<img src="https://img-blog.csdnimg.cn/img_convert/33c00a1933a8fbdf47db211178125621.png" alt="image-20200704145340513" style="zoom:50%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/c56a02961383aca01f642eb9a1b81ab1.png" alt="image-20210507100104455" style="zoom:50%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/8a37d7f7b848e7453e622270217ce7dc.png" alt="img" style="zoom:50%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/cd8b795cf080b8ba40f6915b6242c8af.png" alt="image-20210507104030823"  />
...
- 指令流中的指令大部分是`零地址指令`，其执行过程依赖于操作栈。指令集更小，编译器容易实现
...
Java 虚拟机的启动是通过引导类加载器（bootstrap class loader）创建一个初始类（initial class 名字不一定 - 但是注意不是Object类）来完成的，这个类是由虚拟机的具体实现指定的。
...
> ### ‌[永久代](https://www.baidu.com/s?tn=15007414_9_dg&wd=永久代&usm=2&ie=utf-8&rsv_pq=d24ca842002839b6&oq=永久代&rsv_t=2cba6rBzrCOeUpJ8ViBA0Qgm%2F5mo725vff7HDlzR3JSqm%2Bpm2TgXOz9shlXiYNfIWQ1DUg&sa=re_dqa_generate)的定义和作用‌
>
> 我们经常说的方法区**只是java虚拟机规范的一个概念**，就像是一个接口；那么接口总要有人实现吧，所以永久代和元空间就起着这样的作用，它们实际上都是方法区，是方法区在不同jdk版本的实现。
>
> 在‌[Java](https://www.baidu.com/s?tn=15007414_9_dg&wd=Java&usm=2&ie=utf-8&rsv_pq=d24ca842002839b6&oq=永久代&rsv_t=8447ZF7bh3v35IoVDZYngMfoQ5It8d3QIT8BrlfuMxCS%2F9R1PTH9FRtvfaoX7mY50kcJQw&sa=re_dqa_generate)虚拟机（‌[JVM](https://www.baidu.com/s?tn=15007414_9_dg&wd=JVM&usm=2&ie=utf-8&rsv_pq=d24ca842002839b6&oq=永久代&rsv_t=e25fb6YfzXlI%2BaefiftmOqVVerFpWACSwqsrjy6OialYMyH%2FP1JtgOz3gcVXYLS58ACK%2Bg&sa=re_dqa_generate)）中，‌**永久代**‌（PermGen space）是‌[JDK 7](https://www.baidu.com/s?tn=15007414_9_dg&wd=JDK 7&usm=2&ie=utf-8&rsv_pq=d24ca842002839b6&oq=永久代&rsv_t=c8aeoir0xp%2BHZSCPh1MMLKCQzIjCdEZgrLrlNIJ8dXDzI4ZE1GtPwITEQSOSUysaoRsiHg&sa=re_dqa_generate)及以前版本中用于存储类的元数据（如类定义、常量、静态变量等）的内存区域。它属于JVM的‌[堆内存](https://www.baidu.com/s?tn=15007414_9_dg&wd=堆内存&usm=2&ie=utf-8&rsv_pq=d24ca842002839b6&oq=永久代&rsv_t=a208kIq8NrJpC0vdJeAeX%2FjQ0s1zPyTF7n4ixRr%2FhtIRHMJhfzZr1tLhgl8VFn3GRxdnYA&sa=re_dqa_generate)的一部分，但与堆内存分开管理。
>
> ### 永久代的实现方式
>
> 在JDK 7及以前版本中，‌**永久代**‌是使用‌**永久代（PermGen space）**‌来实现的。从JDK 8开始，永久代被‌**‌[元空间](https://www.baidu.com/s?tn=15007414_9_dg&wd=元空间&usm=2&ie=utf-8&rsv_pq=d24ca842002839b6&oq=永久代&rsv_t=96aeOjv98IBTH7vcFg34bm%2FG7v7qEYCH81jpN4EIraO%2FcRxTNgrny2Ur8i4T%2B5SieMERHA&sa=re_dqa_generate)（Metaspace）**‌所替代。元空间通过使用本地内存来存储类的元数据，解决了永久代的一些问题，如内存溢出和垃圾回收效率低下。
>
> ### 永久代的替代方案
>
> 从JDK 8开始，‌**元空间（Metaspace）**‌被引入作为永久代的替代方案。元空间使用本地内存来存储类的元数据，不再使用JVM的堆内存。这解决了永久代的一些问题，如内存溢出和垃圾回收效率低下。
>
> ### 永久代的问题和影响
>
> 使用永久代时，可能会遇到内存溢出问题。当加载过多的类或常量时，永久代的空间可能会不足，导致抛出`java.lang.OutOfMemoryError: PermGen space`异常。此外，永久代中的垃圾回收效率较低，可能会影响应用程序的性能。

### JRockit

- <mark>专注于服务器端应用</mark>
  - `它可以不太关注程序启动速度`，因此 JRockit 内部不包含解析器实现，全部代码都靠`即时编译器`编译后执行。
