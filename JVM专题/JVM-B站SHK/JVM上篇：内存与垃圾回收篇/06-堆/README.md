...
> 可视化工具JVM - jvisualvm
>
> `jvisualvm` 早期JDK之前自带，后期的JDK需要下载。 `apt/brew install visualvm` 

## 6.1. 堆（Heap）的核心概述

堆针对一个 JVM 进程来说是唯一的，也就是一个进程只有一个 JVM，但是进程包含多个线程，他们是共享同一堆空间的。

<img src="https://img-blog.csdnimg.cn/img_convert/0be60cad417bedd46d651f710ae585ce.png" alt="image-20200706195127740" style="zoom: 67%;" />
...
- `堆内存的大小是可以调节的。`
- `-Xms[num][k/m/g]`初始堆空间大小 `-Xmx[num][k/m/g]`最大堆空间
...
/**
* 1. 設置雄空间大小的参数
*-Xms 用来设置雄空间（年轻代+老年代）的初始内存大小
*-X是jvm的运行参数
* ms 是memory start
*-Xmx 用来设置雄空间（年轻代＋老年代）的最大内存大小

* 2. 默认雄空间的大小
* 初始内存大小：物理电脑内存大小 / 64
最大内存大小：物理电脑内存大小 / 4

3. 手动設置：-Xms600m -Xmx600m
开发中建议将初始雄内存和最大的雄内存設置成相同的值。

4. 查看設置的参数：方式一：jps / jstat -gc 进程id
...
[The java Command (oracle.com)](https://docs.oracle.com/en/java/javase/17/docs/specs/man/java.html)

`可以通过 jinfo 命令查看比例参数是多少`
...
## 6.5. Minor GC (YGC)，Major GC (针对老年代)、Full GC

`GC运行时，会导致 STW stop the world 导致用户线程暂停。`
...
<img src="https://img-blog.csdnimg.cn/img_convert/29146da9c1f3a3241d52201641899020.png" alt="image-20200707095606813" style="zoom:67%;" />
...
-XX:+PrintFlagsFinal  //查看所有的参数的最终值（可能会存在修改，不再是初始值）
  具体查看某个参数的指令：jps：查看当前运行中的进程
											jinfo -flag SurvivorRatio ifFid
-Xms  //初始堆空间内存（默认为物理内存的1/64）
-Xmx  //最大堆空间内存（默认为物理内存的1/4）
-Xmn  //设置新生代的大小。（初始值及最大值）
-XX:NewRatio  //配置新生代与老年代在堆结构的占比
-XX:SurvivorRatio  //设置新生代中Eden和S0/S1空间的比例  过大 容易进入老年代 过小 频繁YGC
...
- 当一个对象在方法中被定义后，对象只在方法内部使用，则认为没有发生逃逸。=> `则可使用 栈上分配`
...
- 参数`-XX:+DoEscapeAnalysis`：启用逃逸分析（启用之后，才会考虑使用栈上分配）
