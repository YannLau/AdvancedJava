...
> jdk的垃圾回收的串行回收器、并行回收器、并发回收器具体含义
>
> ChatGPT 说：
> 在Java中，垃圾回收器（Garbage Collector, GC）用于自动管理内存，确保不再使用的对象被回收。不同类型的垃圾回收器在工作方式和适用场景上有所不同。以下是串行回收器、并行回收器和并发回收器的具体含义：
>
> 串行回收器（Serial Garbage Collector）：
>
> 工作方式：串行回收器在进行垃圾回收时，会暂停应用程序的执行（称为"Stop-the-world"）。它使用单线程进行回收过程。
> 适用场景：适合于单核处理器或者小型应用程序，因为它的实现简单，开销小，但在多核系统上可能会成为瓶颈。
> 并行回收器（Parallel Garbage Collector）：
>
> 工作方式：并行回收器也会在回收期间暂停应用程序的执行，但使用多个线程并行处理垃圾回收。这种方式可以加速回收过程。
> 适用场景：适合于多核处理器的环境，尤其是在需要处理大量数据或长时间运行的应用程序中，可以显著提高性能。
> 并发回收器（Concurrent Garbage Collector）：
>
> 工作方式：并发回收器在进行垃圾回收时，不会完全暂停应用程序的执行，而是与应用程序并发执行。这意味着回收过程可以在应用程序运行的同时进行，从而减少停顿时间。
> 适用场景：适合于对延迟敏感的应用程序，如实时系统或需要持续响应的用户界面应用程序。通过并发回收，可以在保持应用程序响应性的同时进行内存管理。
> 这些回收器各有优缺点，选择适合的回收器可以根据应用的需求、硬件环境和性能要求来决定。
...
6. JDK8 中 Parallel Scavenge GC 和 Parallel Old GC 是默认方案。
7. ParallelScavengeGC 不能和 CMSGC 搭配的原因是使用的框架不同不兼容。
...
在JDK8中，指定 +XX:UseParallelGC 指的是使用新生代的Parallel Scavenge GC，此时会自动触发 +XX:UseParallelOldGC参数。
...
ParNew和其搭档（Serial Old GC）的绑定在新版本14中已经被移除了，称为了孤家寡人。另一个搭档 CMS GC在JDK14直接被移除了。
...
- `-XX:ParallelGCThreads` 设置年轻代并行收集器的线程数。一般地，最好与 CPU 数量相等，以避免过多的线程数影响垃圾收集性能。
  $$
  \operatorname{ParallelGCThreads} = \begin{cases} 
  	CPU\_Count & \text (CPU\_Count <= 8) \\ 
  	3 + (5 * \frac{CPU\_Count}{8}) & \text (CPU\_Count > 8) \end{cases}
  $$
...
- `-XX:GCTimeRatio` 垃圾收集时间占总时间的比例（=$\frac{1}{N+1}$）。用于衡量吞吐量的大小。
...
- `-XX:+UseAdaptivesizePolicy` 设置 Parallel Scavenge 收集器具有<mark>自适应调节策略</mark>
  ...
  - 伊甸园区和S0、S1区的比例默认： SerialGC使用的默认比例是8:1，parallel使用的是6:1
  - 新生代和老年代的默认比例为 1:2

## 13.6. CMS 回收器：低延迟（低暂停时间）
...
尽管 CMS 收集器采用的是并发回收（非独占式），但是<mark>在其初始化标记和再次标记这两个阶段中仍然需要执行“Stop-the-World”机制</mark>暂停程序中的工作线程，不过暂停时间并不会太长，因此可以说明`目前所有的垃圾收集器都做不到完全不需要“stop-the-World”`，只是尽可能地缩短暂停时间。
...
<img src="https://img-blog.csdnimg.cn/img_convert/9a71df3a4013da274aa3b28cd95e7d37.png" alt="image-20200713215105293" style="zoom:67%;" />

<img src="https://img-blog.csdnimg.cn/img_convert/8bcd36541731eb308043eba968b7a828.png" alt="image-20200713215133839" style="zoom:67%;" />
...
从经验上来说，在小内存应用上 CMS 的表现大概率会优于 G1，而 G1 在大内存应用上则发挥其优势。平衡点在 6GB-8GB 之间。
...
- `-XX:+ParallelGCThread` 设置 STW GC 工作线程数的值。最多设置为 8（上面说过 Parallel 回收器的线程计算公式，当 CPU_Count > 8 时，ParallelGCThreads 也会大于 8）
...
<img src="https://img-blog.csdnimg.cn/img_convert/049520c5a004b63f750e04d5362a2992.png" alt="image-20200713223509993" style="zoom: 67%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/c97de4bdada1da5a9fdd344692ed0957.png" alt="image-20200713224716715" style="zoom:67%;" />
...
<img src="https://img-blog.csdnimg.cn/img_convert/f29fa4dfe3abf4a77be06fdf3378aecf.png" alt="image-20200713225100632" style="zoom:67%;" />
...
```java
Heap（堆）
PSYoungGen（Parallel Scavenge收集器新生代）total 9216K(伊甸园区和其中一个Survivor区的和)，used 6234K [0x00000000ff600000,0x0000000100000000,0x0000000100000000)
...
<img src="https://img-blog.csdnimg.cn/img_convert/8dada6b73786eb693975593db10ce825.png" alt="image-20200714083332238" style="zoom:67%;" />
