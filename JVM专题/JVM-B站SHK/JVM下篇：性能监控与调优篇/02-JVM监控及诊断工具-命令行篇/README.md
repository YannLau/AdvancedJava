...
`RMI 注册表`中注册的主机名。如果想要远程监控主机上的 java 程序，需要安装 jstatd。
...
1、在linux服务器未开启jstatd服务时：本地主机使用`jps ip` 直接报错

```java
RMI Registry not available at 192.168.2.2:1099
error during JRMP connection establishment; nested exception is:
	java.io.EOFException
```

2、在远程主机上的jdk的bin目录中创建文件`jstatd.all.policy`
...
3、给这个文件赋权 `chmod +x jstatd.all.policy`

4、启动jstatd服务

```java
 ./jstatd -J-Djava.rmi.server.hostname=192.168.2.2 \                                                       
         -J-Djava.rmi.server.port=1099 \
         -J-Djava.rmi.server.useLocalHostname=false \
         -J-Djava.security.policy=jstatd.all.policy
```

5、记得要关闭防火墙ufw/firewalld，因为这个服务是RMI,它不仅仅在1099端口监听, `netstat -anp` 可以看到jstat监听好几个端口。因此要关闭防火墙，或者手动开放他监听的所有端口！
